...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'jabber-alert-message-hooks 'howdy-jabber-message-received-hook)
#+END_SRC
***** Hook up phone logs
#+BEGIN_SRC emacs-lisp
  ;; fixme: pull out the xml from the zip.

  (loop for item in (cddadr  (xml-parse-file "/tmp/calllog.xml" t))
        do (when (listp item)
             (let ((phone (cdr (assoc 'Number (cadr item))))
               ...
               (when (string= direction "INCOMING")
                 (howdy--contacted `((:phone . ,phone)) time)))))

  ;; fixme: hook this up to run as a cron...
  ;; fixme: do the same for sms logs.
#+END_SRC
***** Howdy from agenda
#+BEGIN_SRC emacs-lisp
