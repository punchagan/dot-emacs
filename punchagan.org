...
#+BEGIN_SRC emacs-lisp
  ...
  (set-register ?c `(file . ,(expand-file-name "contacts.txt" org-directory)))
#+END_SRC

*** Code to fix contacts file                                     :ARCHIVE:
#+BEGIN_SRC emacs-lisp
  ...
  ;; (org-map-entries 'pc/org-contact-fix-in-buffer org-contacts-matcher
...
******* Received mail
#+BEGIN_SRC emacs-lisp
  (advice-add 'mu4e-headers-view-message :before 'pc/howdy-mu4e-message-receive)

  (defun pc/howdy-mu4e-message-receive ()
...
******* Sent mail
#+BEGIN_SRC emacs-lisp
  (add-hook 'message-send-hook 'pc/howdy-mu4e-message-send)

  (defun pc/howdy-mu4e-message-send ()
...
******* Sent messages
#+BEGIN_SRC emacs-lisp
  (add-hook 'jabber-chat-send-hooks 'pc/howdy-jabber-message-send)

  (defun pc/howdy-jabber-message-send (body id)
    (let* ((info `((:email . ,jabber-chatting-with))))
      (howdy--contacted info nil)))
#+END_SRC
******* Received messages
#+BEGIN_SRC emacs-lisp
  (add-hook 'jabber-alert-message-hooks 'pc/howdy-jabber-message-received)

  (defun pc/howdy-jabber-message-received (from buffer text title)
    (let ((info `(:email . ,(jabber-jid-user from))))
