...
#+BEGIN_SRC emacs-lisp
  ...
  (defun pc/get-all-contacts-for-tag (tag)
  ...
  (defun pc/get-all-emails-for-tag (tag domains)
    (let ((contacts (pc/get-all-contacts-for-tag tag)))
  ...
  (defun pc/jabber-chat-with ()
    ...
    (let* ((to (completing-read
      ...
      (defun pc/jabber--chat-with (n to &optional msg)
        (if (not (null n))
      ...
      (if (pc/is-email-p to)
        ...
        (let* ((emails (pc/get-all-emails-for-tag to pc/chat-domains))
          ...
          (loop for email in emails
                ...
                do (run-with-timer (* x 1.5) nil 'pc/jabber--chat-with n email msg))))))
